...
friend ostream& operator<<(ostream& output, const DataRebin& rebinner);
  
  Data data;//local copy of the data to rebin
  vector<double> rebin;//to store the rebin result
  double* rebinArray;//rebin array in a C-style version
  static vector<double> GetCommonElements(const vector<double>& v1, const vector<double>& v2, const double epsilon = 1e-3);
  static vector<int> GetCommonIndices(const vector<double>& v1, const vector<double>& v2, const double epsilon = 1e-3);//find the indices in v1 whose values match that of v2
...
public:
  ...
  DataRebin(const Data& data, const vector<double>& rebin);//pass the rebin vector and check if it is admissible, otherwise, overlook it
  DataRebin(const DataRebin& other);
  DataRebin& operator = (const DataRebin& other);
  ~DataRebin();
  void ApplyRebin();//actually rebin the Histograms
  const Data& GetData() const;
  const vector<double>& GetRebin() const;
